...
//________________________________________________________________________
AliXiStarpp13TeV::AliXiStarpp13TeV(const char *name, Bool_t AODdecision,  Bool_t MCdecision, Int_t CutListOption, Bool_t DevelopmentMode, Bool_t HMTrigger)
...
void AliXiStarpp13TeV::UserCreateOutputObjects()
{
    ...
    TH1F *fMultDist_pp_afterPileUpReject = new TH1F("fMultDist_pp_afterPileUpReject","Multiplicity Distribution of PP",1000,0,1000);
    fMultDist_pp_afterPileUpReject->GetXaxis()->SetTitle("Multiplicity");
...
void AliXiStarpp13TeV::Exec(Option_t *)
{
    ...
    // Muliplicity Check (old way, before event cut)
    ...
    // Muliplicity Check (new way, after several event cut)
    //if(fESD->IsPileupFromSPD()) return; // Reject Pile-up events, old way.
    lPerc = 500; //nonsense
    ...
    Int_t lEvSelCode = MultSelection->GetEvSelCode();
    //if(lEvSelCode > 0) lPerc = lEvSelCode; //disregard!
    if(lEvSelCode == 0) lPerc = lEvSelCode + 500;
    
    if(fDevelopeMode)std::cout << "Multiplicity(after Pile-up rejection): " << lPerc << std::endl;
